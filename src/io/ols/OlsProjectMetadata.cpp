                                              ...
                                              QStringList *warnings)
{
    ...
    // ── Always-read CString fields (matches ALWAYS_READ_FIELDS in parse_ols.py) ──
    ...
    // ── Schema-gated fields (matches SCHEMA_GATED_FIELDS in parse_ols.py) ──
    // Helper lambdas that check the schema gate, read, and handle errors.

    auto gatedString = [&](uint32_t minVer, QString &dst, const char *name) -> bool {
        if (!ar.hasField(minVer)) return true;   // not present, but OK
        if (ar.eof()) {
            if (warnings)
                warnings->append(
    ...
    // Read gated fields in the exact order specified by parse_ols.py
