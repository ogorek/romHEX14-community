...
public:
    ...
    QString    cstring();          // u32 len + bytes + NUL (NUL absent if len==0)
    QByteArray cstringBytes();     // same but returns raw bytes
    QByteArray byteArray();        // u32 len + raw bytes (no NUL)
    QVector<uint64_t> u64Array();  // u32 count + count*8 bytes
    QByteArray bulk(qsizetype n);  // read n bytes, no prefix
