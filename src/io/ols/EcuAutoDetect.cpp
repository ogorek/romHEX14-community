...
// ── Signature table ──────────────────────────────────────────────────────────
//
// Ported byte-for-byte from portal/dtc/app.py (ECU_SIGNATURES).  The list
// order is meaningful: more-specific entries come first because firmware
// images frequently embed broader ECU family names as substrings (e.g. an
// MED17 image will contain the literal "EDC17" inside an A2L blob — the
// MED17 entry must therefore match first).
...
static const QVector<SigRow> &signatures()
{
    static const QVector<SigRow> kSigs = {
        // Bosch petrol — checked BEFORE diesel EDC patterns
        {"MED17",   "",     {"MED17", "MED 17"}},
        {"MEDC17",  "",     {"MEDC17"}},
        {"MEV17",   "",     {"MEV17", "MEV 17"}},
        {"MEVD17",  "",     {"MEVD17"}},
        ...
        // Bosch EDC17 sub-models — specific first
        ...
        // Bosch EDC16 — specific before generic
        ...
        // Bosch EDC15 / EDC7 / DCU / PSG / MT35 / ME155
...
static QString producerForFamily(const QString &family)
{
    static const QHash<QString, QString> kMap = {
        ...
        // Denso (& Denso-built ECUs of Toyota/Mazda/Subaru)
...
// ── Part-number scanners ────────────────────────────────────────────────────
// Real ROMs rarely embed a "Hardware: " marker outright — the part numbers
// usually sit as bare strings in an ID block.  Scan the ROM for the typical
// VW/Audi and Bosch part-number shapes and return the first match.
...
// VW/Audi HW: 3 alnum + "9" + 5 digits + 1..3 uppercase letters.
//   examples: "038906019FE", "8K1907115M", "06A906018R", "8N0906018BH"
static QString findVwHwNumber(const QByteArray &data, qsizetype maxScan)
{
    ...
    for (qsizetype i = 0; i + 11 < maxScan; ++i) {
        ...
        // Trailing 1..3 uppercase letters (revision)
        int len = 10;
...
// Bosch ECU HW: "026X" or "028X" prefix + 6..7 alnum.
//   examples: "0261B04884", "0281010744", "0261207751", "0261206776"
...
// Bosch SW number — two known formats:
//   A) "1037" + 6 digits        e.g. "1037391128", "1037505704"
//   B) "026XS" + 5 digits       e.g. "0261S04137"
...
// Production number — exactly 6 digits, isolated word.  Common values lie in
// 100000..999999 (smaller numbers are too noisy).  Used as a tertiary fill.
...
// ── HW/SW marker scan (secondary pass) ──────────────────────────────────────
// Look for "Hardware: <part-no>" / "HW: <...>" / "Software: <...>" / "SW: ..."
// markers and return the trimmed value (terminated by NUL, CR, LF or 32 chars).
...
EcuDetectionResult EcuAutoDetect::detect(const QByteArray &data)
{
    ...
    // Constrain search to first ~3 MiB; modern ECU images embed identifying
    // strings in the calibration descriptor area near the start of flash.
    const qsizetype searchEnd = std::min<qsizetype>(data.size(), 0x300000);

    // ── Pass 1: signature table ─────────────────────────────────────────
    for (const auto &row : signatures()) {
        for (const auto &pat : row.patterns) {
            ...
            // Capture context bytes for diagnostics
            ...
            // Best-effort HW/SW from named markers (rare in raw ROMs but
            // present in A2L-prefixed images and OLS exports).
            ...
            // Fallback: scan for bare part-number patterns embedded in the
            // ROM's ID block.  Most VAG / Bosch images carry both a VW HW
            // (e.g. "03L906023A") AND a Bosch HW (e.g. "0261B04884") — we
            // map the VW one to hwNumber (the field WinOLS uses) and the
            // Bosch one to hwAltNumber for diagnostics.
    ...
    // ── Pass 2: derive family from a Hardware-marker value ──────────────
...
// ── Inventory helpers (UI dropdown population) ──────────────────────────────
...
// Reverse-lookup: turn a "Build" string from the project ("EDC17C46",
// "MEVD17 2.6", "SIMOS 18") back into the (family, variant) pair that
// PatternEngine / DtcEngine consume.
...
QStringList EcuAutoDetect::knownEcus()
{
    static const QStringList kCached = []() {
        ...
        for (const auto &row : signatures()) {
            const QString family = QString::fromLatin1(row.family);
            const QString model  = QString::fromLatin1(row.model);
            // Concat family+model with a separating space when model is
            // dotted/multi-digit (MEVD17 2.6, ME17 3.3, DCM 6.2).  For
            // alphanumeric model suffixes (EDC17C46, SIMOS18) keep them
