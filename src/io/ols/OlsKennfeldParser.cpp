/*
 * This file is part of romHEX14.
 * Copyright (C) 2026 Cristian Tabuyo <contact@romhex14.com>
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

// Schema-driven Kennfeld (calibration map) record parser for OLS .ols files.
//
// Exact port of extract_maps.py — the Python reference implementation.
// Every field is read at its schema-determined position.  No heuristic
// scanning for scale doubles, no sentinel-based dimension guessing.
//
// See KENNFELD_RECORD_SCHEMA.md for the byte-level wire schema.

#include "OlsKennfeldParser.h"

#include <QRegularExpression>
#include <QSet>
#include <QStringDecoder>
#include <QtEndian>
#include <cmath>
#include <cstring>
#include <limits>

namespace ols {

// ── Sanity caps from  ─────────────────────────────────────
static constexpr uint32_t DIM_A276_MAX  = 0x4000;
static constexpr uint32_t DIM_A396_MAX  = 0x400;
static constexpr uint32_t DIM_A400_MAX  = 0x1F400;

// Cell width -> bytes per cell.  Mirrors the Python reference
// (extract_maps.py decode_axis_breakpoints / decode_cells):
//   cell_bits=2  -> 1 byte  (4-bit-per-nibble packed; one byte per cell slot)
//   cell_bits=8  -> 1 byte
//   cell_bits=10 -> 2 bytes (10-bit packed into a 16-bit slot)
//   cell_bits=16 -> 2 bytes
//   cell_bits=32 -> 4 bytes
// Anything else defaults to 2 bytes (16-bit Bosch cells).
static int cellBitsToBytes(uint32_t cb)
{
    switch (cb) {
    case 2:  return 1;
    case 8:  return 1;
    case 10: return 2;
    case 16: return 2;
    case 32: return 4;
    default: return 2;
    }
}

// ── Inline byte helpers ───────────────────────────────────────────────────

uint32_t OlsKennfeldParser::peekU32(const QByteArray &data, qsizetype off)
{
    if (off + 4 > data.size()) return 0;
    return qFromLittleEndian<uint32_t>(
        reinterpret_cast<const uchar *>(data.constData() + off));
}

double OlsKennfeldParser::peekF64(const QByteArray &data, qsizetype off)
{
    if (off + 8 > data.size()) return std::numeric_limits<double>::quiet_NaN();
    uint64_t bits = qFromLittleEndian<uint64_t>(
        reinterpret_cast<const uchar *>(data.constData() + off));
    double v;
    std::memcpy(&v, &bits, sizeof(v));
    return v;
}

// ── String decoder (UTF-8 first, Windows-1252 fallback) ──────────────────
// Same logic as CArchiveReader::cstring(). Newer OLS versions write
// some fields as UTF-8; older ones use Windows-1252.

static QString decodeOlsString(const char *p, int len)
{
    if (len <= 0) return QString();
    QByteArray raw(p, len);
    bool allAscii = true;
    for (char c : raw) {
        if (static_cast<uint8_t>(c) >= 0x80) { allAscii = false; break; }
    }
    if (allAscii) return QString::fromLatin1(raw);

    QStringDecoder utf8(QStringDecoder::Utf8);
    QString asUtf8 = utf8(raw);
    if (!utf8.hasError() && !asUtf8.contains(QChar::ReplacementCharacter))
        return asUtf8;

    QStringDecoder cp1252("Windows-1252");
    if (cp1252.isValid()) return cp1252(raw);

    return QString::fromLatin1(raw);
}

// ── Text / ident checks ──────────────────────────────────────────────────

bool OlsKennfeldParser::isText(const char *data, int length)
{
    if (length <= 0) return false;
    int printable = 0;
    for (int i = 0; i < length; ++i) {
        auto b = static_cast<uint8_t>(data[i]);
        // Accept: printable ASCII + common whitespace + Windows-1252 high range
        // (0x80-0x9F has printable chars in CP1252: €, „, …, –, —, etc.)
        // + Latin-1 supplement (0xA0-0xFF: accented chars, symbols)
        if ((b >= 32 && b < 127) || b == 0x09 || b == 0x0A || b == 0x0D
            || b >= 0x80)   // accept ALL high bytes (UTF-8 continuations + CP1252)
            ++printable;
    }
    return printable >= static_cast<int>(0.85 * length);
}

bool OlsKennfeldParser::isIdent(const QString &s)
{
    static const QRegularExpression re(
        QStringLiteral("^[A-Za-z_][A-Za-z0-9_]{0,40}$"));
    return re.match(s).hasMatch();
}

// ── CString reader ────────────────────────────────────────────────────────
// Port of extract_maps.py read_cstring_ols().
// Wire format (schema < 439):
//   [u32 length][length+1 bytes]  when length > 0  (the +1 is the NUL)
//   [u32 length=0]                when length == 0  (no body)

OlsKennfeldParser::PStr OlsKennfeldParser::readCString(
    const QByteArray &data, qsizetype offset, int maxLen, int schema)
{
    PStr result;
    if (offset + 4 > data.size()) return result;

    // Read signed length to handle schema-439 interned sentinels.
    int32_t slen = static_cast<int32_t>(peekU32(data, offset));

    if (schema >= 439) {
        // Modern: no trailing NUL; -1/-2/-3 are interned tokens.
        if (slen == -1) {
            result.text = QStringLiteral("-");
            result.endOffset = offset + 4;
            return result;
        }
        if (slen == -2) {
            result.text = QStringLiteral("?");
            result.endOffset = offset + 4;
            return result;
        }
        if (slen == -3) {
            result.text = QStringLiteral("%");
            result.endOffset = offset + 4;
            return result;
        }
        if (slen <= 0) {
            result.text = QLatin1String("");
            result.endOffset = offset + 4;
            return result;
        }
        if (slen > maxLen) return result;
        if (offset + 4 + slen > data.size()) return result;
        const char *p = data.constData() + offset + 4;
        if (!isText(p, slen)) return result;
        result.text = decodeOlsString(p, slen);
        result.endOffset = offset + 4 + slen;   // NO trailing NUL
        return result;
    }

    // Classic: [u32 length][bytes][NUL] when length > 0.
    if (slen < 0 || slen > maxLen) return result;
    uint32_t len = static_cast<uint32_t>(slen);
    if (len == 0) {
        result.text = QLatin1String("");
        result.endOffset = offset + 4;
        return result;
    }
    if (offset + 4 + static_cast<qsizetype>(len) + 1 > data.size())
        return result;
    const char *p = data.constData() + offset + 4;
    if (!isText(p, static_cast<int>(len))) return result;
    result.text = decodeOlsString(p, static_cast<int>(len));
    result.endOffset = offset + 4 + len + 1;  // consume the trailing NUL
    return result;
}

// ── CFolderRef reader () ──────────────────────────────────
//
// Wire layout (verified against  at
// addresses  through ):
//
//   schema < 330:  CString name (single field)
//
//   schema >= 330: CString name
//                + (if schema >= 345) u32  → a2+8   [first  read]
//                + (if schema >= 345) u32  → a2+8   [second read]
//                + u32 count (always when schema >= 330)
//                + (if schema >= 345 and count > 0) for each:
//                      CString + u32
//
// The two consecutive `if (schema >= 345) call C2C270` blocks
// addresses  and  both write to a2+8 (a union
// overlay).  On the wire this is **8 bytes** (two u32) when schema >= 345,
// not 4.  Missing this read offsets the parser by 4 bytes per Kennfeld in
// v440+ files (BMW F10/F11/F18, Mazda 6 Denso) — symptom is "0 maps".
//
// Mirrors extract_maps.py read_cfolderref() (Python reference impl).
struct CfrResult {
    QString name;
    qsizetype endOffset = -1;
    bool valid() const { return endOffset >= 0; }
};

static CfrResult readCFolderRef(const QByteArray &data, qsizetype offset,
                                int schema)
{
    CfrResult result;
    auto inner = OlsKennfeldParser::readCString(data, offset, 8192, schema);
    if (!inner.valid()) return result;
    result.name = inner.text;
    qsizetype o = inner.endOffset;
    if (schema < 330) {
        result.endOffset = o;
        return result;
    }
    // schema >= 330: TWO consecutive `if (schema >= 345)` u32 reads,
    // then the always-read u32 count.
    if (schema >= 345) {
        if (o + 4 > data.size()) return result;
        o += 4;  // 1st u32 → a2+8 ()
    }
    if (schema >= 345) {
        if (o + 4 > data.size()) return result;
        o += 4;  // 2nd u32 → a2+8 ()
    }
    if (o + 4 > data.size()) return result;
    uint32_t count = OlsKennfeldParser::peekU32(data, o);
    o += 4;
    if (count > 0x1000) return result;
    if (schema >= 345 && count > 0) {
        for (uint32_t i = 0; i < count; ++i) {
            auto entry = OlsKennfeldParser::readCString(data, o, 8192, schema);
            if (!entry.valid()) return result;
            o = entry.endOffset;
            if (o + 4 > data.size()) return result;
            o += 4;  // per-entry u32
        }
    }
    result.endOffset = o;
    return result;
}

// ── Envelope blocklist ────────────────────────────────────────────────────
//
// UNIVERSALITY NOTE: only OLS-emitted control strings belong here. Vehicle
// make/model/ECU-name/tuner-name strings vary per project and MUST NOT be
// blocklisted — doing so silently drops maps with those exact comments on
// every other ECU than Seat Leon. The previous version of this set contained
// customer-specific tokens like "Seat", "Leon", "MED17.5.20", "Bosch",
// "Eprom", "Westy" inherited from the Seat Leon reference sample and has
// been pruned.
static const QSet<QString> &envelopeStrings()
{
    static const QSet<QString> s = {
        // OLS UI / serializer-emitted category and version labels.
        // These are written by OLS itself for every project (not by the
        // user) and appear in the project envelope before the maps region.
        QStringLiteral("(DAMOS)"),
        QStringLiteral("OLS 5.0 (Windows)"),
        QStringLiteral("Hexdump"), QStringLiteral("My maps"),
        QStringLiteral("Original"),
        QStringLiteral("Passenger car"),
        QStringLiteral("Engine"),
        QStringLiteral("Binary data"),
        QStringLiteral("0 (Original)"),
        QStringLiteral("Version 1"),
    };
    return s;
}

// ── Cursor: sequential byte walker ────────────────────────────────────────
// Port of extract_maps.py Cursor class.

struct Cursor {
    const QByteArray &buf;
    qsizetype off;
    qsizetype start;
    int schema = 288;

    Cursor(const QByteArray &b, qsizetype o, int sch = 288)
        : buf(b), off(o), start(o), schema(sch) {}

    qsizetype remain() const { return buf.size() - off; }

    uint8_t u8() {
        auto v = static_cast<uint8_t>(buf.at(off));
        off += 1;
        return v;
    }

    uint32_t u32() {
        auto v = OlsKennfeldParser::peekU32(buf, off);
        off += 4;
        return v;
    }

    double f64() {
        auto v = OlsKennfeldParser::peekF64(buf, off);
        off += 8;
        return v;
    }

    bool bool1() {
        auto v = static_cast<uint8_t>(buf.at(off));
        off += 1;
        return v != 0;
    }

    void skip(int n) { off += n; }

    // Read a CString at current position; returns empty PStr on failure.
    OlsKennfeldParser::PStr cstr() {
        auto r = OlsKennfeldParser::readCString(buf, off, 8192, schema);
        if (r.valid())
            off = r.endOffset;
        return r;
    }

    // Read a CFolderRef at current position; returns invalid CfrResult on failure.
    CfrResult cfr() {
        auto r = readCFolderRef(buf, off, schema);
        if (r.valid())
            off = r.endOffset;
        return r;
    }
};

// ── _classify_dims ────────────────────────────────────────────────────────
// Port of extract_maps.py _classify_dims().

static QPair<int,int> classifyDims(uint32_t kennfeldType,
                                   uint32_t a276, uint32_t a280,
                                   uint32_t a396, uint32_t a400)
{
    // Sanity clamp
    if (a396 > DIM_A396_MAX) a396 = 0;
    if (a400 > DIM_A400_MAX) a400 = 0;
    if (a276 > DIM_A276_MAX) a276 = 0;

    if (kennfeldType == 1 || kennfeldType == 2) {
        // Scalar
        return {1, 1};
    }
    if (kennfeldType == 3) {
        // 1D curve: prefer a396, fall back to max(a276, a280)
        int x;
        if (a396 > 0) {
            x = static_cast<int>(a396);
        } else {
            x = static_cast<int>(qMax(a276, a280));
            if (x <= 0) x = 1;
        }
        return {x, 1};
    }
    if (kennfeldType == 5) {
        // 2D map: a396=Y, a400=X from CC70; fall back to a280=X, a276=Y
        if (a400 > 0 || a396 > 0) {
            int x = (a400 > 0) ? static_cast<int>(a400) : 1;
            int y = (a396 > 0) ? static_cast<int>(a396) : 1;
            return {x, y};
        }
        int x = (a280 > 0) ? static_cast<int>(a280) : 1;
        int y = (a276 > 0) ? static_cast<int>(a276) : 1;
        return {x, y};
    }
    // Unknown type: try CC70 first, then a276/a280
    if (a400 > 0 && a396 > 0)
        return {static_cast<int>(a400), static_cast<int>(a396)};
    if (a280 > 0 || a276 > 0)
        return {qMax(static_cast<int>(a280), 1), qMax(static_cast<int>(a276), 1)};
    return {1, 1};
}

// ── _read_axis ────────────────────────────────────────────────────────────
// Port of extract_maps.py _read_axis().

struct AxisResult {
    AxisInfo axis;
    qsizetype endOffset = -1;
    bool valid() const { return endOffset >= 0; }
};

static AxisResult readAxis(const QByteArray &buf, qsizetype axisStart,
                           int schema, int expectedSize)
{
    AxisResult result;
    if (axisStart + 4 > buf.size())
        return result;

    qsizetype o = axisStart;

    // 1. CFolderRef axis_name (a2+0).  For schema < 330 this is a single
    //    CString; for schema >= 330 the full folder hierarchy is serialized
    //    (and for schema >= 345 the two pre-count u32 reads MUST also be
    //    consumed — see readCFolderRef above).  Empty / sentinel-string
    //    axis_name values are valid (scalar maps still emit two axis
    //    sub-records); the caller decides whether to surface the axis
    //    based on the map's display dimensions.
    auto cfr = readCFolderRef(buf, o, schema);
    if (!cfr.valid()) return result;
    QString axisName = cfr.name;
    o = cfr.endOffset;

    // 2. CString axis_unit
    auto r = OlsKennfeldParser::readCString(buf, o, 8192, schema);
    if (!r.valid()) return result;
    QString axisUnit = r.text;
    o = r.endOffset;

    // 3. f64 axis_scale
    if (o + 8 > buf.size()) return result;
    double scale = OlsKennfeldParser::peekF64(buf, o); o += 8;

    // 4. f64 axis_offset
    if (o + 8 > buf.size()) return result;
    double offsetV = OlsKennfeldParser::peekF64(buf, o); o += 8;

    // 5. Five u32s: axis_type, ROM_addr, axis_data_type (u32_116), u32_120, cell_bits
    //    AXIS_CELLWIDTH_SCHEMA.md: a2+116 (axis_data_type) is the AUTHORITATIVE
    //    byte-layout enum; a2+124 (cell_bits) is DISPLAY-only.
    if (o + 5 * 4 > buf.size()) return result;
    uint32_t axisType       = OlsKennfeldParser::peekU32(buf, o); o += 4;
    uint32_t axisRomAddress = OlsKennfeldParser::peekU32(buf, o); o += 4;
    uint32_t axisDataType   = OlsKennfeldParser::peekU32(buf, o); o += 4;  // u32_116
    /* u32_120 */             OlsKennfeldParser::peekU32(buf, o); o += 4;
    uint32_t cellBits       = OlsKennfeldParser::peekU32(buf, o); o += 4;

    if (cellBits != 2 && cellBits != 10 && cellBits != 16)
        cellBits = 10;

    // 6. Two bools (a2+128 = flag1, a2+129 = flag2 [inverse])
    if (o + 2 > buf.size()) return result;
    /* flag1 */ o += 1;
    /* flag2 */ o += 1;

    // ===== schema-gated optional tail =====

    // gate 264: f64 extra_88 (inverse-mode divisor)
    if (schema >= 264) {
        if (o + 8 > buf.size()) return result;
        o += 8;
    }
    // gate 241: u32
    if (schema >= 241) {
        if (o + 4 > buf.size()) return result;
        o += 4;
    }
    // gate 8: u32 extra_140 (decimal digits for cell_bits==10 display)
    if (schema >= 8) {
        if (o + 4 > buf.size()) return result;
        o += 4;
    }
    // gate 12: bool flag3 (signed flag — sign-extend raw cell)
    bool flag3 = false;
    if (schema >= 12) {
        if (o + 1 > buf.size()) return result;
        flag3 = (static_cast<uint8_t>(buf[o]) != 0);
        o += 1;
    }
    // gate 73: CByteArray (u32 count + count bytes)
    if (schema >= 73) {
        if (o + 4 > buf.size()) return result;
        uint32_t inlineCount = OlsKennfeldParser::peekU32(buf, o); o += 4;
        if (inlineCount > 0) {
            if (inlineCount > 0x10000 || o + static_cast<qsizetype>(inlineCount) > buf.size())
                return result;
            o += inlineCount;
        }
    }
    // gate 77: u32
    if (schema >= 77) {
        if (o + 4 > buf.size()) return result;
        o += 4;
    }
    // gate 91: u32 axis_rom_v2
    uint32_t axisRomV2 = 0;
    if (schema >= 91) {
        if (o + 4 > buf.size()) return result;
        axisRomV2 = OlsKennfeldParser::peekU32(buf, o); o += 4;
    }
    // gate 354: CString
    if (schema >= 354) {
        r = OlsKennfeldParser::readCString(buf, o, 8192, schema);
        if (!r.valid()) return result;
        o = r.endOffset;
    }
    // gate 372: array of CFolderRef.  Wire layout matches :
    //   [u32 count][count× CFolderRef]
    // The count is read raw via  (no helper interpretation).
    // We MUST consume this on the wire even when count==0, otherwise the
    // cursor for the next axis sub-record (and the gate-440 u32 below) is
    // offset by 4 + (count × CFolderRef-size) bytes.  Missing this read
    // is what kept BMW F10 (v440) and Mazda 6 (v440) returning 0 maps
    // before this fix.
    if (schema >= 372) {
        if (o + 4 > buf.size()) return result;
        uint32_t folderCount = OlsKennfeldParser::peekU32(buf, o);
        o += 4;
        if (folderCount > 0x1000) return result;
        for (uint32_t i = 0; i < folderCount; ++i) {
            auto fr = readCFolderRef(buf, o, schema);
            if (!fr.valid()) return result;
            o = fr.endOffset;
        }
    }
    // gate 440: u32
    if (schema >= 440) {
        if (o + 4 > buf.size()) return result;
        o += 4;
    }

    // Resolve ROM address: prefer the unconditional wire field;
    // fall back to gate-91 v2.
    uint32_t romAddr = axisRomAddress;
    if (!romAddr || romAddr == 0xFFFFFFFF) {
        if (axisRomV2 && axisRomV2 != 0xFFFFFFFF)
            romAddr = axisRomV2;
        else
            romAddr = 0;
    }

    Q_UNUSED(axisType);

    // Map axis_data_type → bytes-per-cell + endianness flag.
    // Mirror of  in the OLS serializer.
    auto bytesFromDataType = [](uint32_t dt) -> int {
        switch (dt) {
        case 2: case 3:                   return 2;
        case 4: case 5: case 6: case 7:   return 4;
        case 10: case 11: case 12: case 13: return 8;
        default: return 1;  // {0, 1, 8, 9} and any unknown → u8
        }
    };
    auto isBigEndianDataType = [](uint32_t dt) -> bool {
        // 2 = u16 BE, 3 = u16 LE; 4 = u32 BE, 5 = u32 LE; 6 = float BE, 7 = float LE; etc.
        return (dt == 2 || dt == 4 || dt == 6 || dt == 10 || dt == 12);
    };

    result.axis.inputName = axisName;
    result.axis.scaling.unit = axisUnit;
    result.axis.hasScaling = (scale != 1.0 || offsetV != 0.0);
    result.axis.scaling.type = CompuMethod::Type::Linear;
    result.axis.scaling.linA = scale;
    result.axis.scaling.linB = offsetV;
    result.axis.hasPtsAddress = (romAddr > 0 && romAddr != 0xFFFFFFFF);
    result.axis.ptsAddress = romAddr;
    result.axis.ptsCount = expectedSize;
    result.axis.ptsDataType = axisDataType;
    result.axis.ptsBigEndian = isBigEndianDataType(axisDataType);
    result.axis.ptsSigned = flag3;
    // ptsDataSize is derived from data_type when available; fall back to the
    // legacy cellBitsToBytes() mapping for axes without an explicit data_type
    // (which shouldn't happen for OLS files, but keeps the field
    // populated for downstream code that still consults it).
    result.axis.ptsDataSize = (axisDataType > 0)
        ? bytesFromDataType(axisDataType)
        : cellBitsToBytes(cellBits);
    result.endOffset = o;
    return result;
}

// ── parse_record ──────────────────────────────────────────────────────────
// Port of extract_maps.py parse_record().

MapInfo OlsKennfeldParser::parseOne(const QByteArray &data,
                                     qsizetype commentOffset,
                                     qsizetype hardLimit,
                                     int schema)
{
    MapInfo mi;
    Cursor c(data, commentOffset, schema);

    // ===== 1. COMMENT (CFolderRef — schema-dependent layout) =====
    // For schema < 330 this reduces to a single CString; for schema >= 330
    // the full folder hierarchy is serialized (and the schema >= 345 path
    // adds two pre-count u32 reads — see readCFolderRef above).  Walking
    // the full CFolderRef is REQUIRED for v440+ files (BMW F10, Mazda 6
    // Denso) — using readCString here directly drops 4 bytes per record
    // and the parser fails on every Kennfeld.
    auto cm = c.cfr();
    if (!cm.valid()) return mi;
    if (envelopeStrings().contains(cm.name)) return mi;
    if (cm.name.startsWith(QStringLiteral("Imported from"))) return mi;
    if (!cm.name.isEmpty() && !isText(cm.name.toLatin1().constData(), cm.name.size()))
        return mi;

    // ===== 2. 5 unconditional u32s =====
    // a152 = AUTHORITATIVE cell data_type enum ( reads
    // `*(_DWORD *)(a4 + 152)` and feeds it to the byte-decode switch).
    // Same enum table as axis ptsDataType.  See AXIS_CELLWIDTH_SCHEMA.md.
    if (c.remain() < 20) return mi;
    uint32_t kennfeldType  = c.u32();   // a2+72
    c.u32();                            // a148
    uint32_t cellDataType  = c.u32();   // a2+152 — cell data_type enum
    c.u32();                            // a160
    uint32_t cellBits      = c.u32();   // a164  (DISPLAY only)
    if (cellBits != 2 && cellBits != 10 && cellBits != 16)
        cellBits = 10;

    // ===== 3. NAME block (schema >= 80) =====
    QString name;
    if (schema >= 80) {
        if (c.remain() < 8) return mi;
        c.u32();                       // a84
        auto n = c.cstr();             // NAME
        if (!n.valid() || !isIdent(n.text)) return mi;
        name = n.text;
    } else {
        return mi;  // older schemas: can't identify record
    }

    // ===== 4. Optional u32s after NAME =====
    if (schema >= 298) c.u32();        // a88
    if (schema >= 299) c.u32();        // a284
    if (schema >= 94)  c.u32();        // a92
    if (schema >= 74)  c.bool1();      // a80

    // ===== 5. Subtype-dependent u32 (a192) =====
    // Inner archive subtype defaults to 1; gate 123 fires.
    if (schema >= 123)
        c.u32();                       // a192 (subtype-1)

    // ===== 6. Doubles section =====
    if (schema >= 300) {
        for (int i = 0; i < 6; ++i) c.f64();             // 6x C2C2D0
        if (schema >= 67) {
            for (int i = 0; i < 6; ++i) c.skip(8);       // 6x C33590 discarded
        }
    } else if (schema >= 67) {
        for (int i = 0; i < 6; ++i) c.skip(8);           // 6x C33590
    } else if (schema >= 66) {
        c.skip(16);                                       // 2x C33590
    } else if (schema < 59) {
        c.skip(8);                                        // 2x int
    }
    // else (schema in [59, 66)): no bytes read

    // ===== 7. 4 bools (always) =====
    // Per schema:  (the byte-decoder):
    //   a4+260 → inverse flag      (v112 → triggers `1.0/(raw+inv_div)` mode)
    //   a4+261 → SIGNED flag       (v95  → triggers sign-extension cast)
    //   a4+262 → diff-vs-reference (v94)
    //   a4+263 → another mode toggle
    bool cellInverse = c.bool1();   // a260
    bool cellSigned  = c.bool1();   // a261
    c.bool1();                      // a262
    c.bool1();                      // a263
    Q_UNUSED(cellInverse);          // not currently surfaced in MapInfo

    // ===== 8. Geometry u32s (always) =====
    uint32_t a276 = c.u32();
    uint32_t a280 = c.u32();
    uint32_t a396 = c.u32();
    uint32_t a400 = c.u32();
    c.u32();  // a408

    auto [xSize, ySize] = classifyDims(kennfeldType, a276, a280, a396, a400);
    if (xSize > static_cast<int>(DIM_A400_MAX) || ySize > static_cast<int>(DIM_A396_MAX))
        return mi;
    if (xSize <= 0 || ySize <= 0)
        return mi;

    // ===== 9. Labels + SCALE/OFFSET + ROM address =====
    auto a568 = c.cstr();  // label
    auto a576 = c.cstr();  // unit
    if (!a568.valid() || !a576.valid()) return mi;

    double scale  = c.f64();
    double offset = c.f64();
    uint32_t romAddress = c.u32();   // a624 = ROM data start
    uint32_t a640 = c.u32();         // a640 = ROM data end
    c.u32();                         // a644

    QString physUnit = !a576.text.isEmpty() ? a576.text : a568.text;

    // ===== 10. Schema-gated fields before axes =====
    if (schema >= 264) c.f64();        // a600
    if (schema >= 61)  c.u32();        // a628
    if (schema >= 105) {
        c.u32(); c.u32(); c.u32(); c.u32();  // a608, a612, a616, a620
    }

    // ===== 11. Axis sub-records (always TWO, on-disk order = rows then cols) =====
    // Per KENNFELD_RECORD_SCHEMA.md:
    //   (_, a2+648)    → ROWS axis (vertical, Y in display)
    //   (_, a2+1000)   → COLS axis (horizontal, X in display)
    // So the first sub-record on disk goes to yAxis (rows, ySize breakpoints)
    // and the second goes to xAxis (cols, xSize breakpoints).
    auto rowsAxis = readAxis(data, c.off, schema, ySize);
    if (!rowsAxis.valid()) return mi;
    c.off = rowsAxis.endOffset;

    auto colsAxis = readAxis(data, c.off, schema, xSize);
    if (!colsAxis.valid()) return mi;
    c.off = colsAxis.endOffset;

    // ===== 12. Post-axis tail: find 01 01 01 trailer for record boundary =====
    qsizetype trailerSearchStart = c.off;
    qsizetype trailerSearchEnd = qMin(hardLimit, trailerSearchStart + 0x800);
    qsizetype trailer = -1;
    {
        const char *d = data.constData();
        for (qsizetype i = trailerSearchStart; i + 3 <= trailerSearchEnd; ++i) {
            if (static_cast<uint8_t>(d[i]) == 0x01
                && static_cast<uint8_t>(d[i+1]) == 0x01
                && static_cast<uint8_t>(d[i+2]) == 0x01) {
                trailer = i;
                break;
            }
        }
    }

    qsizetype recordEnd = (trailer >= 0) ? trailer + 3 : c.off;

    // ===== Build MapInfo =====
    mi.name = name;
    mi.description = cm.name;
    mi.rawAddress = romAddress;
    mi.address = romAddress;  // caller adjusts for base if needed
    mi.dimensions = { xSize, ySize };

    // ── CELL byte-layout (AUTHORITATIVE) ─────────────────────────────────
    // Mirror the data_type → bytes-per-cell mapping from 
    // and the byte-decode switch in .  cellDataType
    // (kennfeld+152) is the OLS-internal enum that drives ROM reads
    // for cells; cellBits (kennfeld+164) is DISPLAY-only and must NOT be
    // used to determine ROM cell width (same lesson as axes — see
    // AXIS_CELLWIDTH_SCHEMA.md).
    //
    // When cellDataType == 0 (older OLS files or non-OLS sources
    // like A2L), fall back to the legacy ROM-range heuristic so importers
    // that can't supply the enum keep working.
    auto bytesFromDataType = [](uint32_t dt) -> int {
        switch (dt) {
        case 2: case 3:                     return 2;
        case 4: case 5: case 6: case 7:     return 4;
        case 10: case 11: case 12: case 13: return 8;
        default: return 1;  // {0, 1, 8, 9} and any unknown → u8
        }
    };
    auto isBigEndianDataType = [](uint32_t dt) -> bool {
        return (dt == 2 || dt == 4 || dt == 6 || dt == 10 || dt == 12);
    };

    int derivedCellBytes;
    if (cellDataType != 0) {
        derivedCellBytes = bytesFromDataType(cellDataType);
    } else {
        // Legacy fallback — only triggered when OLS didn't write a
        // data_type (very old schemas) or the producer is A2L.
        derivedCellBytes = cellBitsToBytes(cellBits);
        uint32_t romEnd = a640;   // a640 = ROM address end
        if (romAddress > 0 && romEnd > romAddress && xSize > 0 && ySize > 0) {
            uint32_t dataBytes = romEnd - romAddress;
            int totalCells = xSize * ySize;
            if (totalCells > 0) {
                int computedSize = static_cast<int>(dataBytes) / totalCells;
                if (computedSize == 1 || computedSize == 2 || computedSize == 4)
                    derivedCellBytes = computedSize;
            }
        }
    }
    mi.dataSize       = derivedCellBytes;
    mi.cellDataType   = cellDataType;
    mi.cellBigEndian  = isBigEndianDataType(cellDataType);
    // Sign-extension flag is the AUTHORITATIVE bool at kennfeld+261
    // (v95 in ).  When cellDataType == 0 (legacy path) we
    // fall back to the historical heuristics: 1-byte u8 cells with
    // cellBits=10 use signed i8 (ignition-timing convention); else any
    // negative-offset map is treated as signed.
    if (cellDataType != 0) {
        mi.dataSigned = cellSigned;
    } else {
        mi.dataSigned = (derivedCellBytes == 1 && cellBits == 10)
                        || (offset < 0);
    }
    mi.linkConfidence = 100;
    // Bosch ECUs store map data column-major: each column (all Y values
    // for one X position) is contiguous in ROM. OLS reads them this way.
    mi.columnMajor = true;

    // Type classification
    if (xSize <= 1 && ySize <= 1)
        mi.type = QStringLiteral("VALUE");
    else if (ySize <= 1)
        mi.type = QStringLiteral("CURVE");
    else
        mi.type = QStringLiteral("MAP");

    // Scaling
    if (scale != 0.0 && scale != 1.0) {
        mi.hasScaling = true;
        mi.scaling.type = CompuMethod::Type::Linear;
        mi.scaling.linA = scale;
        mi.scaling.linB = offset;
    } else if (offset != 0.0) {
        mi.hasScaling = true;
        mi.scaling.type = CompuMethod::Type::Linear;
        mi.scaling.linA = (scale != 0.0) ? scale : 1.0;
        mi.scaling.linB = offset;
    }
    if (!physUnit.isEmpty())
        mi.scaling.unit = physUnit;

    // Axes — first sub-record = rows (Y), second = cols (X)
    mi.xAxis = colsAxis.axis;
    if (ySize > 1)
        mi.yAxis = rowsAxis.axis;

    // Store record_end for walker to use (stash in length field)
    mi.length = static_cast<int>(recordEnd - commentOffset);

    return mi;
}

// ── Walk all records ──────────────────────────────────────────────────────
// Port of extract_maps.py walk_records().

QVector<MapInfo> OlsKennfeldParser::parseAll(const QByteArray &data,
                                              qsizetype regionStart,
                                              qsizetype regionEnd,
                                              int schema,
                                              QStringList *warnings)
{
    Q_UNUSED(warnings);
    QVector<MapInfo> maps;
    qsizetype off = regionStart;

    while (off < regionEnd - 8) {
        MapInfo mi = parseOne(data, off, regionEnd, schema);
        if (!mi.name.isEmpty()) {
            qsizetype recordEnd = off + mi.length;
            // We stashed (recordEnd - commentOffset) in mi.length as a scratch
            // walker offset. Now replace it with the REAL byte length of the
            // map payload (dims × dataSize) — the waveform widget uses this
            // to size the coloured map-region rectangles on the 2D view.
            // Anything that leaves length == 0 makes every region invisible.
            {
                const int cells = qMax(1, mi.dimensions.x)
                                * qMax(1, mi.dimensions.y);
                mi.length = cells * qMax(1, mi.dataSize);
            }
            maps.append(mi);
            off = recordEnd;
            // Skip inter-record padding (typically <16 bytes)
            for (int skip = 0; skip < 64 && off < regionEnd; ++skip) {
                if (off + 4 > data.size()) break;
                uint32_t peek = peekU32(data, off);
                if (peek >= 4 && peek <= 200)
                    break;
                ++off;
            }
            continue;
        }
        ++off;
    }

    return maps;
}

// ── Find map region ───────────────────────────────────────────────────────
// Port of extract_maps.py find_map_region().

QPair<qsizetype, qsizetype> OlsKennfeldParser::findMapRegion(
    const QByteArray &data, int schema)
{
    qsizetype end = data.size();

    // The map region ends at the first TAG_POSTZIP_HDR (0x98638811).
    char needle[4];
    qToLittleEndian<uint32_t>(0x98638811, reinterpret_cast<uchar *>(needle));
    qsizetype j = data.indexOf(QByteArray::fromRawData(needle, 4));
    if (j > 0)
        end = j;

    // Find first valid record by scanning forward from 0x180.
    qsizetype start = 0x180;
    qsizetype scanEnd = qMin(static_cast<qsizetype>(0x10000), end - 32);
    for (qsizetype off = 0x180; off < scanEnd; ++off) {
        if (off + 4 >= end) break;
        uint32_t L = peekU32(data, off);
        if (L >= 4 && L <= 200 && off + 4 + static_cast<qsizetype>(L) < end) {
            const char *p = data.constData() + off + 4;
            if (!isText(p, qMin(static_cast<int>(L), 32)))
                continue;
            MapInfo test = parseOne(data, off, end, schema);
            if (!test.name.isEmpty()) {
                start = off;
                break;
            }
        }
    }
    return {start, end};
}

// ── KP intern payload parser ──────────────────────────────────────────────

QVector<MapInfo> OlsKennfeldParser::parseIntern(const QByteArray &internPayload,
                                                 int schema,
                                                 QStringList *warnings)
{
    if (internPayload.size() < 14) {
        if (warnings)
            warnings->append(OlsKennfeldParser::tr("intern payload too small (%1 bytes)")
                                 .arg(internPayload.size()));
        return {};
    }

    const uint32_t mapCount = peekU32(internPayload, 1);
    if (mapCount == 0 || mapCount > 100000) {
        if (warnings)
            warnings->append(OlsKennfeldParser::tr("intern payload map count %1 out of range")
                                 .arg(mapCount));
        return {};
    }

    QVector<MapInfo> maps;
    qsizetype off = 14;
    const auto sz = internPayload.size();

    while (off < sz - 8) {
        // Skip record marker bytes (0x00)
        while (off < sz && static_cast<uint8_t>(internPayload.at(off)) == 0x00) ++off;
        if (off >= sz - 8) break;

        MapInfo mi = parseOne(internPayload, off, sz, schema);
        if (!mi.name.isEmpty()) {
            qsizetype recordEnd = off + mi.length;
            mi.length = 0;
            maps.append(mi);
            off = recordEnd;
            continue;
        }
        ++off;
    }

    return maps;
}

} // namespace ols
