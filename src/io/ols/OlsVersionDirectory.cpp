...
// Magic anchor M1 / "Version-record fence" — the marker we scan for
// in the first ~2 KB of the file.  The directory header sits at
// anchorOffset - 12 (3 u32s precede the magic itself).
                                                ...
                                                QStringList *warnings)
{
    ...
    // ── 1. Locate the M1 anchor in the preamble ────────────────────────
    // The LOAD wrapper reads <= 1008 bytes then scans.  We search the
    // full preamble region (first 4 KB to be safe).
    ...
    // ── 2. Read the 3 u32s immediately before the anchor ────────────────
    // Layout: [numVersions-1][versionDataStart][versionRecordSize][MAGIC]
    // The anchor is at anchorPos; the header starts at anchorPos - 12.
    ...
    // ── 3. Sanity check ─────────────────────────────────────────────────
    // versionDataStart + 4 + versionRecordSize * numVersions == fileSize
        ...
        if (warnings)
            warnings->append(
                QStringLiteral("version-directory sanity: expected file size %1, got %2")
                    .arg(expectedEnd)
                    .arg(fileData.size()));
        // Continue anyway — some files have appended data
    }
