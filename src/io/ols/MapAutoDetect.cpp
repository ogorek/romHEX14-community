...
// ── Cell readers ─────────────────────────────────────────────────────────────
// Read one cell (u8 / u16 LE-BE / u32 LE-BE) as double.
                         ...
                         bool bigEndian, bool isSigned)
{
    ...
    // Need at least 80% of steps to be strict (catch "0,0,0,0,5" = filler+1).
    if (strictSteps * 5 < (count - 1) * 4) return 0;

    // Base 55 + length bonus (max 30) + range realism (10) + strict bonus (5)
    double s = 55.0;
    ...
    while (off + cellBytes * minLen <= n
        ...
        while (count < maxLen
            ...
            const double cur = readCell(p + off + count * cellBytes,
                                         cellBytes, bigEndian, isSigned);
            // Weakly monotonic: allow equality (== prev), break on direction reversal.
...
// ── Non-max suppression ──────────────────────────────────────────────────────
// When two candidates cover overlapping byte ranges, keep the higher-scored
// one.  Catches the same data block reinterpreted under different cellBytes
// or layouts, plus accidental sub-block matches inside larger maps.
...
static QVector<MapCandidate> suppressOverlaps(QVector<MapCandidate> cands)
{
    ...
    for (const auto &c : cands) {
        ...
        for (const auto &k : kept) {
            ...
            // Drop if our region is >=50% covered by an already-kept one.
...
{
    // Weighted combo + small penalty for unusual cell sizes.
                                           ...
                                           const MapAutoDetectOptions &opts)
{
    ...
    // ── 1. Hunt axes (u8 + u16 LE always; u16 BE optional) ─────────────
    ...
    auto tryEmit2D = [&](const AxisHit &ax, const AxisHit &ay,
        ...
        // Cells that don't match the axis cellBytes get a small penalty.
    ...
    // ── 2. Phase A: 2D maps — axes precede block (Bosch layout) ────────
    ...
    // ── 3. Phase B: 2D maps — block precedes axes (Audi/VAG layout) ────
    for (int i = 0; i < axes.size()
                    && result.size() < opts.maxCandidatesPerRegion; ++i) {
        const AxisHit &ax = axes[i];
    ...
    // ── 4. Phase C: 1D curves — axis followed by N values ──────────────
    for (int i = 0; i < axes.size()
                    && result.size() < opts.maxCandidatesPerRegion; ++i) {
        const AxisHit &ax = axes[i];
        const int axEnd = ax.offset + ax.count * ax.cellBytes;
    ...
    // ── 5. Phase D: 1D curves — N values followed by axis ──────────────
    for (int i = 0; i < axes.size()
                    && result.size() < opts.maxCandidatesPerRegion; ++i) {
        const AxisHit &ax = axes[i];
        const int N = ax.count;
    ...
    // ── 6. Suppress overlaps + final sort ───────────────────────────────
    // Convert to ROM-relative addresses for the suppress step (so we don't
    // miss overlaps that would be invisible after baseAddress addition).
