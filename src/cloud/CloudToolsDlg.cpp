...
{
    bool ok = false;
    QString token = QInputDialog::getText(this, tr("Cloud configuration"),
        tr("API token  (leave empty to drop back to the free DTC tier):"),
        QLineEdit::Password, m_client->proToken(), &ok);
    if (!ok) return;
    m_client->setProToken(token);

    refreshAuthBadge();
    m_status->setText(token.isEmpty()
        ? tr("Token cleared — free tier active.")
        : tr("Token saved — Pro tier active."));
}
